    ...
    async import({file_location, client_info}) {
        ...
        const {accountTemplate} = await sequelize.transaction(async (t) => {
            const templateDetails = {
                name: 'Template 1', country: 'India', sector: 'IT', createdBy, organizationId
            };
            const newAccountTemplate = await AccountsTemplateDetailsDao.create({template_details: templateDetails}, {transaction: t});
            ...
            const all_accounts_details = [
                {
                    name: "Asset", code: "1", accountTemplateId: newAccountTemplateId, type: "group"
                }, {
                    name: "Equity", code: "3", accountTemplateId: newAccountTemplateId, type: "group"
                }, {
                    name: "Liability", code: "2", accountTemplateId: newAccountTemplateId, type: "group"
                },
                {
                    name: "Income", code: "4", accountTemplateId: newAccountTemplateId, type: "group"
                },
                {
                    name: "Expense", code: "5", accountTemplateId: newAccountTemplateId, type: "group"
                }, {
                    name: "Gain Or Loss", code: "6", accountTemplateId: newAccountTemplateId, type: "group"
                }
            ].map(acc => ({...acc, createdBy, organizationId}));
            rawEntries.filter(acc => acc.name).forEach((acc) => {
            ...
            await AccountsOfTemplateDao.createAccountsFromDump({account_template_id: newAccountTemplateId}, {transaction: t})
